...
const { hasOwn } = require('../../type-utils')
...
ObjectTyped.keys(nextLogger.prefixes).forEach(
  (nextLoggerMethod) => {
    if (
      !hasOwn(
    ...
    const prefixedChildLogger =
      createPrefixedChildLogger({
        nextLoggerMethod,
        pinoLoggerMethod,
      })

    const rawPinoLogFn = prefixedChildLogger

    /** @type {import('../types').supportedConsoleMethods} */
    // @ts-expect-error - I'm being lazy (jk - under pressure at work)
    const methodPrefix = nextLoggerMethod
    ...
    const wrappedPinoLogFn = (
      ...
      return rawPinoLogFn(
        createStrictArgShufflerForPino(methodPrefix)(
          args,
        ),
      )
    }
    ...
    // Q: can we 'bind' correct `prefix:` onto each childLogger method?
    // and thus create ONE childLogger instance?
    nextLogger[nextLoggerMethod] = wrappedPinoLogFn
  },
)
