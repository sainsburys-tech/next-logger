...
describe('config', () => {
  ...
  const runScriptInDockerWithConfig = async (
    script,
    preset,
  ) => {
  ...
  beforeAll(async () => {
    ...
    const configFilePath = path.resolve(
      __dirname,
      'config/next-logger.config.js',
    )
    container = await builder
      .withBindMount(
        configFilePath,
        '/app/next-logger.config.js',
        'ro',
      )
      .withCmd(['top'])
      .start()
  ...
  it('loads a config from the working directory and uses that Pino instance', async () => {
    const script = buildNextJsScript(
      'info',
      "'Message for info'",
    )
    const { stdout, exitCode } =
      await runScriptInDockerWithConfig(
        script,
        undefined,
      )
