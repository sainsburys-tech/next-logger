  ...
  describe.each(scenarios)(
    ...
    (_, script, expected) => {
      it('prints a JSON style string to stdout', async () => {
        const { stdout } = await scriptRunner(
          script,
          preset,
        )
      ...
      it('prints nothing to stderr', async () => {
        const { stderr } = await scriptRunner(
          script,
          preset,
        )
      ...
      it('logs a timestamp close to Date.now()', async () => {
        ...
        expect(log.time).toBeGreaterThanOrEqual(
          now - acceptableMargin,
        )
        expect(log.time).toBeLessThanOrEqual(
          now + acceptableMargin,
        )
      })

      it('logs the PID of the process', async () => {
        const { stdout /* , pid: expectedPid */ } =
          await scriptRunner(script, preset)
